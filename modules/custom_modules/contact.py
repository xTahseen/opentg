...
async def addx_contact(c: Client, message: Message):
    try:
        ...
        name_candidate = args[0] if (args := message.text.split(maxsplit=1)[1:]) else (
        ...
        if user.is_mutual_contact:
            ...
            if message.reply_to_message or args:
                await c.send_message(message.chat.id, "beautiful name")
                await asyncio.sleep(1)
                await c.send_message(message.chat.id, "can you take circle video? wanna see you.")
            else:
                await asyncio.sleep(1)
            ...
            await message.edit(
                f"<b>Contact removed (not mutual):</b> <a href='tg://user?id={user.id}'>{user.first_name}</a>"
            )
            await asyncio.sleep(1)
